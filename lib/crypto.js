...
exports.verify = function verify(alg, msg, sig, key) {
  ...
  switch (key.alg) {
    case 'rsa': {
      ...
      try {
        pub = rsa.publicKeyImport(key.data);
      } catch (e) {
        return false;
      }

      return rsa.verify(hash, hash.digest(msg), sig, pub);
    }
...
exports.sign = function sign(alg, msg, key) {
  ...
  switch (key.alg) {
    case 'rsa': {
      const priv = rsa.privateKeyImport(key.data);
      return rsa.sign(hash, hash.digest(msg), priv);
    }
